...
void graph_add_dot(void* address, double value, int type, void* left, void* right, const char* fillcolor)
{
    if(type == NUMBER)
    ...
    else if(type == ADD || type == SUB || type == MUL || type == DIV || type == POW || type == LN || type == SIN || type == COS)
        ...
        fprintf(get_log_file(".dot"), "node%p [shape = Mrecord, fillcolor = \"%s\",style = filled, color = \"#000000\", label = \"{value = %lf | type = unknown | address = %p | left = %p | right = %p}\"];\n", address, fillcolor, value, address, left, right);
}
